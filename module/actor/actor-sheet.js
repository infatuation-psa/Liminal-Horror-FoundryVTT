 ...
 * @extends {ActorSheet}
 */
export class LiminalActorSheet extends ActorSheet {
  /** @override */
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      classes: ["liminal", "sheet", "actor"],
      template: "systems/liminal/templates/actor/actor-sheet.html",
      width: 480,
      height: 640,
      tabs: [
        {
      ...
      dragDrop: [{ dragSelector: ".liminal-items-list-row", dropSelector: null }],
    });
  }

  get template() {
    const path = "systems/liminal/templates/actor";
  ...
  activateListeners(html) {
    ...
    html.find(".item-edit").click((ev) => {
      const li = $(ev.currentTarget).parents(".liminal-items-list-row");
  ...
  async _onItemCreate(event) {
    ...
    const template = "systems/liminal/templates/dialog/add-item-dialog.html";
    const content =  await renderTemplate(template);

    new Dialog({
      title: game.i18n.localize("LIMINAL.CreateItem"),
      content,
      buttons: {
        create: {
          icon: '<i class="fas fa-check"></i>',
          label: game.i18n.localize("LIMINAL.CreateItem"),
          callback: (html) => {
  ...
  async _onAddFatigue(event) {
    ...
    this.actor.createOwnedItem({
      name: game.i18n.localize("LIMINAL.Fatigue"),
      type: 'item'
    });
  }
  ...
  async _onRemoveFatigue(event) {
    ...
    const fatigues = this.actor.items
      .filter(i => i.name === game.i18n.localize("LIMINAL.Fatigue"));
  ...
  async _onRoll(event) {
    ...
    if (dataset.roll) {
      ...
      const label = dataset.label ? game.i18n.localize("LIMINAL.Rolling") + ` ${dataset.label}` : "";
  ...
  _buildDamageRollMessage(label, targetIds) {
    const rollMessageTpl = 'systems/liminal/templates/chat/dmg-roll-card.html';   
  ...
  _onItemDescriptionToggle(event) {
    event.preventDefault();
    const boxItem = $(event.currentTarget).parents(".liminal-items-list-row");
  ...
  async _onRegenerateCharacter(event) {
    ...
    const confirm = await Dialog.confirm({
      title: game.i18n.localize("LIMINAL.CharacterRegeneratorTitle"),
      content: `<p>${game.i18n.localize("LIMINAL.CharacterRegeneratorConfirm")}</p>`,
      defaultYes: false,
    });
      ...
      return [
        {
          ...
          label: game.i18n.localize("LIMINAL.RegenerateCharacter"),
          icon: "fas fa-skull",
