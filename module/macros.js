 ...
 * @return {Promise.<void>}
 */
export const createLiminalMacro = async (data, slot) => {
  ...
  const command = `game.liminal.rollItemMacro("${actor.id}", "${item.id}");`;
...
export const rollItemMacro = async (actorId, itemId) => {
  ...
  const flavor = `${game.i18n.localize("LIMINAL.Rolling")} ${item.stress && <span className="stress">stress </span>}damage with ${item.name}`;
